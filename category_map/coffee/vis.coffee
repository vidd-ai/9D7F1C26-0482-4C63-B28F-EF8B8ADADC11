...
Bubbles = () ->
  ...
  click = (d) ->
    ...
    user='9D7F1C26-0482-4C63-B28F-EF8B8ADADC11';
    # TODO: remove references to fn and figure out how to extract $user
    fn='GOPR20291467058179821high.MP4';
    summary_type='Cat';
    location = 'catView.html';
    #url = "../viewer.html?v=" + location+ "&fn=" + fn + "&u=" + user + "&c=" + summary_type + '&l=' + encodeURIComponent(idValue(d).toLowerCase());
    url = "../viewer.html?v=" + location+ "&fn=" + fn + "&u=" + user + "&c=" + summary_type + '&l=' + idValue(d).split(' ').join('_').toLowerCase()
  ...
  updateActive = (id) ->
    ...
    s = '<span class="text">your Vidd category map<br/>X segments in Y categories';
    d3.select("#status").html(s);
    if id.length > 0
      s += "<span class='text'>The word <span class=\"active\">#{id}</span> is now active</span>";
